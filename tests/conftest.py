...
def test_database():
    ...
    with conn.cursor() as cur:
        cur.execute("DROP TABLE IF EXISTS portfolio_value_history, trades, assistant_requests, "
                    "price_intraday, price_history, portfolio, stocks, users CASCADE")
...
def db(test_database):
    ...
    with cursor(commit=True) as cur:
        cur.execute(
            ...
            "TRUNCATE portfolio_value_history, trades, assistant_requests, price_intraday, "
            "price_history, portfolio, stocks, users RESTART IDENTITY CASCADE"
        )
